...
func (beacon *Beacon) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header) error {
	...
	// Verify existence / non-existence of activitiesHash.
	if shanghai && header.ActivitiesHash == nil {
		return errors.New("missing activitiesHash")
	}
	if !shanghai && header.ActivitiesHash != nil {
		return fmt.Errorf("invalid activitiesHash: have %x, expected nil", header.ActivitiesHash)
	}

	// Verify existence / non-existence of transactionsCount.
	if shanghai && header.TxCount == nil {
		return errors.New("missing transactionsCount")
	}
	if !shanghai && header.TxCount != nil {
		return fmt.Errorf("invalid transactionsCount: have %x, expected nil", header.TxCount)
	}
...
func (beacon *Beacon) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt, withdrawals []*types.Withdrawal) (*types.Block, error) {
	...
	if shanghai {
		return block.WithActivitiesRoot(receipts, trie.NewStackTrie(nil)).WithTxCount(uint64(len(txs))), nil
	}
