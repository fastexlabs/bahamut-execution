...
func (s *BlockChainAPI) GetBlockActivities(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash) (*BlockActivitiesResult, error) {
	...
	for _, receipt := range receipts {
		activities = append(activities, receipt.Activities...)
	}
...
func RPCMarshalHeader(head *types.Header) map[string]interface{} {
	...
	if head.ActivitiesHash != nil {
		result["activitiesRoot"] = head.ActivitiesHash
	}
	if head.TxCount != nil {
		result["transactionsCount"] = head.TxCount
	}
	return result
}
...
func marshalReceipt(receipt *types.Receipt, blockHash common.Hash, blockNumber uint64, signer types.Signer, tx *types.Transaction, txIndex int) map[string]interface{} {
	...
	if receipt.Activities == nil {
		fields["activities"] = []*types.Activity{}
	}
