...
type Genesis struct {
	...
	TxCount *uint64 `json:"transactionsCount"` //FCIP-???
}
...
func ReadGenesis(db ethdb.Database) (*Genesis, error) {
	...
	genesis.TxCount = genesisHeader.TxCount
...
func (g *Genesis) ToBlock() *types.Block {
	...
	if conf := g.Config; conf != nil {
		...
		if conf.IsShanghai(num, g.Timestamp) {
			...
			head.ActivitiesHash = &types.EmptyActivitiesHash
			head.TxCount = g.TxCount
			if head.TxCount == nil {
				head.TxCount = new(uint64)
			}
		}
