...
func CommitGenesisState(db ethdb.Database, hash common.Hash) error {
	...
	if len(blob) != 0 {
		...
		var genesis *Genesis
		switch hash {
		case params.FastexChainTestnetGenesisHash:
...
func DefaultFastexChainTestnetGenesisBlock() *Genesis {
	return &Genesis{
		Config:     params.FastexTestnetConfig,
		ExtraData:  hexutil.MustDecode(""),
		GasLimit:   4700000,
		Difficulty: big.NewInt(1),
		Alloc:      decodePrealloc(fastexChainTestnetAllocData),
	}
}
...
func decodePrealloc(data string) GenesisAlloc {
	var p []struct{ Addr, Balance, Code *big.Int }
	if err := rlp.NewStream(strings.NewReader(data), 0).Decode(&p); err != nil {
		panic(err)
	}
	ga := make(GenesisAlloc, len(p))
	for _, account := range p {
		ga[common.BigToAddress(account.Addr)] = GenesisAccount{Code: account.Code.Bytes(), Balance: account.Balance}
	}
	return ga
}
