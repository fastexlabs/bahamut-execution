...
type Header struct {
	...
	// TODO(fastex): set FCIP ID
	// ActivitiesRoot and TxCount were added by FCIP-??? and is ignored in legacy headers.
	ActivitiesHash *common.Hash `json:"activitiesRoot" rlp:"optional"`
	TxCount        *uint64      `json:"transactionsCount" rlp:"optional"`
...
func CopyHeader(h *Header) *Header {
	...
	if h.ActivitiesHash != nil {
		cpy.ActivitiesHash = new(common.Hash)
		*cpy.ActivitiesHash = *h.ActivitiesHash
	}
	if h.TxCount != nil {
		cpy.TxCount = new(uint64)
		*cpy.TxCount = *h.TxCount
	}
	return &cpy
}
...
func (b *Block) ActivityHash() *common.Hash { return b.header.ActivitiesHash }
func (b *Block) TxCount() *uint64           { return b.header.TxCount }
...
func (b *Block) WithTxCount(txCount uint64) *Block {
	block := &Block{
		header:       b.header,
		transactions: b.transactions,
		uncles:       b.uncles,
...
func (b *Block) WithActivitiesRoot(receipts []*Receipt, hasher TrieHasher) *Block {
	block := &Block{
		header:       b.header,
		transactions: b.transactions,
		uncles:       b.uncles,
	...
	if len(activities) == 0 {
		block.header.ActivitiesHash = &EmptyActivitiesHash
	} else {
		h := DeriveSha(Activities(activities), hasher)
		block.header.ActivitiesHash = &h
	}
