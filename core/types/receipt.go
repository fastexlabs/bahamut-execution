...
func (r *Receipt) EncodeRLP(w io.Writer) error {
	data := &receiptRLP{r.statusEncoding(), r.CumulativeGasUsed, r.Bloom, r.Logs, r.Activities}
...
func (r *Receipt) setFromRLP(data receiptRLP) error {
	r.CumulativeGasUsed, r.Bloom, r.Logs, r.Activities = data.CumulativeGasUsed, data.Bloom, data.Logs, data.Activities
...
func (r *Receipt) Size() common.StorageSize {
	...
	size += common.StorageSize(len(r.Activities)) * common.StorageSize(unsafe.Sizeof(Activity{}))
...
func (r *ReceiptForStorage) EncodeRLP(_w io.Writer) error {
	...
	w.ListEnd(logList)
	activitiesList := w.List()
	for _, activity := range r.Activities {
		// TODO(fastex): check it!
		if err := activity.EncodeRLP(w); err != nil {
			return err
		}
	}
	w.ListEnd(activitiesList)
	w.ListEnd(outerList)
	return w.Flush()
}
...
func (r *ReceiptForStorage) DecodeRLP(s *rlp.Stream) error {
	...
	r.Logs = stored.Logs
	r.Activities = stored.Activities
